    ...
    public partial class PaidByCreditWithPrePaidDebt : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (!hasError)
            {
                ...
                {
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, paymentType, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + (DetailList.Sum(x => x.TotalAmount) * servicefee / 100) - ExtraDiscount, totalCost, null, customerId,SalePerson, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, string.Empty);
                    ...
                    if (totalAmount != 0)
                    {
                        ...
                        if (tList.Count > 0)
                        {
                            ...
                            if (DebtAmount > 0)
                            {
                                System.Data.Objects.ObjectResult<string> DebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Settlement, true, true, 1, 0, 0, DebtAmount, DebtAmount, null, customerId, SalePerson,MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                                entity.SaveChanges();
                            }
                        }
                        else
                    ...
                    if (receiveAmount > 0)
                    {
                        System.Data.Objects.ObjectResult<string> PreDebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Prepaid, true, false, 1, 0, 0, receiveAmount, receiveAmount, null, customerId,SalePerson, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                        entity.SaveChanges();
                    }
                }//end of if
                #endregion
                ...
                else
                {
                    if (totalAmount >= totalCost)
                    {
                        ...
                        {
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, true, true, paymentType, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + (DetailList.Sum(x => x.TotalAmount) * servicefee / 100) - ExtraDiscount - (int)MCDiscount - (int)BDDiscount, receiveAmount, null, customerId, SalePerson,MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                    ...
                    else
                    {
                        if (chkIsPrePaid.Checked == true)
                        {
                            ...
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, false, true, paymentType, ExtraTax + Tax, ExtraDiscount + Discount,
                                 DetailList.Sum(x => x.TotalAmount) + (DetailList.Sum(x => x.TotalAmount) * servicefee / 100) - ExtraDiscount - (int)MCDiscount - (int)BDDiscount, receiveAmount, null, customerId,SalePerson, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent,
                                false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                            totalAmount -= receiveAmount;
                            totalCost -= receiveAmount;
                            resultId = Id.FirstOrDefault().ToString();
                            insertedTransaction = (from trans in entity.Transactions where trans.Id == resultId select trans).FirstOrDefault<Transaction>();
                        ...
                        {
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, false, true, paymentType, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + (DetailList.Sum(x => x.TotalAmount) * servicefee / 100) - ExtraDiscount - (int)MCDiscount - (int)BDDiscount, totalAmount, null, customerId,SalePerson, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                            resultId = Id.FirstOrDefault().ToString();
                            insertedTransaction = (from trans in entity.Transactions where trans.Id == resultId select trans).FirstOrDefault<Transaction>();
                            foreach (TransactionDetail detail in DetailList)
                            {
