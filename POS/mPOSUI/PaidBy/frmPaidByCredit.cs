    ...
    public partial class frmPaidByCredit : Form
    {
        ...
        private void Calculate()
        {
            ...
            if (!hasError)
            {
                ...
                if (totalAmount != 0)
                {
                    if (CreditTransaction.Count > 0)
                    {
                        ...
                        if (DebtAmount > 0)
                        {
                            ...
                            System.Data.Objects.ObjectResult<string> DebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Settlement, true, true, paymentType, 0, 0, DebtAmount, DebtAmount, null, CustomerId,null ,MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, true, joinedTranIdList, IsWholeSale, 0, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
                ...
                if (receiveAmount > 0)
                {
                    System.Data.Objects.ObjectResult<string> PreDebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Prepaid, true, false, paymentType, 0, 0, receiveAmount, receiveAmount, null, CustomerId,null, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, 0, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, transactionParentId);
                    entity.SaveChanges();
                    if (DebtAmount == 0)
                    {
                        entity = new POSEntities();
