...
namespace POS
{
    public partial class PaidByGiftCard : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            long TotalCost = (long)DetailList.Sum(x => x.TotalAmount) + (((long)DetailList.Sum(x => x.TotalAmount) * servicefee) / 100) - ExtraDiscount - (long)BDDiscount - (long)MCDiscount-(long)GiftDiscountAmt;
            long CashReceive = 0;
            ...
            if (GiftCard_Checking())
            {
                ...
                if (!hasError)
                {
                    ...
                    System.Data.Objects.ObjectResult<String> Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.GiftCard, true, true, paymentType, ExtraTax + Tax, ExtraDiscount + Discount, TotalCost, Convert.ToInt32(txtCash.Text), currentCard.Id, CustomerId,SalePerson, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, UsedGiftCardAmt+GiftDiscountAmt, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, null);
