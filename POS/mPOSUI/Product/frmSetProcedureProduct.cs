        ...
        private bool Save()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvSetProduct.Rows)
                {
                    ...
                    product.ProcedureProductID = Convert.ToInt32(cboProcedureProductName.SelectedValue);
                    product.ProcedureProductName = cboProcedureProductName.Text;
                    product.ProductID = Convert.ToInt32(row.Cells[0].Value);
                    product.ProductName = Convert.ToString(row.Cells[1].Value);
                    product.ProductQty = Convert.ToInt32(row.Cells[2].Value);
