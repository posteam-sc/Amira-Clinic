    ...
    public partial class chart : Form
    {
        ...
        private void chart_Load(object sender, EventArgs e)
        {
            ...
            if (itemtopList.Count != 0)
            {
                ...
                {
                    topproduct.Series["Daily_top10product"].Points.AddXY(d.Name, d.totalAmount);
                }
            }
            else
            {
            ...
            foreach (SaleBreakDownController s in FinalResultListCat.OrderByDescending(x => x.saleQty).ToList())
            {
                if (s.BreakDown != 0)
                {
                    salebreakchart.Series["Sale_BreakDown_By_Category"].Points.AddXY(Convert.ToInt32(s.BreakDown) + "%", s.BreakDown);
                }
            }
            foreach (SaleBreakDownController s in FinalResultListCat.OrderByDescending(x => x.saleQty).ToList())
            {
                ...
                {                 
                    ...
                    foreach (DataPoint dp in salebreakchart.Series["Sale_BreakDown_By_Category"].Points)
                    {
                        ...
                        if (dp.YValues[0] ==Convert.ToDouble(s.BreakDown))
                        {
        ...
        public void NetIncome()
        {
            ...
            if (IsStart == true)
            {
                            ...
                            (t.DateTime.Value.Month) == month
                             ...
                             && t.IsActive == true && t.Id.Substring(2, 2) == currentshopcode
                                select new
                                {
                                    ...
                                    ParentId = t.ParentId,
                                    Type = t.Type,
                                    TotalAmt = t.TotalAmount

                                }
                                ).ToList();
                string[] type = { "Refund", "CreditRefund" };
                string[] cashType = { "Sale", "GiftCard" };
                var DtransList = (from t in entity.Transactions where t.DateTime.Value.Month == month && t.DateTime.Value.Year == year && t.IsComplete == true && (t.Type == TransactionType.Settlement || t.Type == TransactionType.Prepaid) && (t.IsDeleted == null || t.IsDeleted == false) && ((currentshopcode != "0" && t.Id.Substring(2, 2) == currentshopcode) || (currentshopcode == "0" && 1 == 1)) select t).ToList<Transaction>();
                var bankSettlementAmt = Convert.ToDecimal((from t in entity.Transactions where t.DateTime.Value.Month == month && t.DateTime.Value.Year == year && t.IsComplete == true && (t.Type == TransactionType.Settlement || t.Type == TransactionType.Prepaid) && (t.IsDeleted == null || t.IsDeleted == false) && (t.PaymentTypeId >= 501) && ((currentshopcode != "0" && t.Id.Substring(2, 2) == currentshopcode) || (currentshopcode == "0" && 1 == 1)) select t.TotalAmount).Sum());
                var bankRecieveCreditAmt = Convert.ToDecimal((from t in entity.Transactions where t.DateTime.Value.Month == month && t.DateTime.Value.Year == year && t.IsComplete == true && t.Type == TransactionType.Credit && (t.IsDeleted == null || t.IsDeleted == false) && (t.PaymentTypeId >= 501) && ((currentshopcode != "0" && t.Id.Substring(2, 2) == currentshopcode) || (currentshopcode == "0" && 1 == 1)) select t.RecieveAmount).Sum());
                var bankRecieveCashAmt = Convert.ToDecimal((from t in entity.Transactions where t.DateTime.Value.Month == month && t.DateTime.Value.Year == year && t.IsComplete == true && (t.Type == TransactionType.Sale || t.Type == TransactionType.GiftCard) && (t.IsDeleted == null || t.IsDeleted == false) && (t.PaymentTypeId >= 501) && ((currentshopcode != "0" && t.Id.Substring(2, 2) == currentshopcode) || (currentshopcode == "0" && 1 == 1)) select t.RecieveAmount).Sum());
                var transList = (from t in entity.Transactions where t.DateTime.Value.Month == month && t.DateTime.Value.Year == year && t.IsComplete == true && t.IsActive == true && (t.Type == TransactionType.Sale || t.Type == TransactionType.GiftCard || t.Type == TransactionType.Credit || t.Type == TransactionType.Refund || t.Type == TransactionType.CreditRefund) && (t.IsDeleted == null || t.IsDeleted == false) && ((currentshopcode != "0" && t.Id.Substring(2, 2) == currentshopcode) || (currentshopcode == "0" && 1 == 1)) select t).ToList<Transaction>();
                var TotalCash = Convert.ToInt64(transList.Where(x => cashType.Contains(x.Type) && x.PaymentTypeId == 1 && !type.Contains(x.Type)).Sum(x => x.RecieveAmount));
                var RecieveCredit = Convert.ToInt64(transList.Where(x => x.Type == "Credit" && !type.Contains(x.Type)).Sum(x => x.RecieveAmount));
                var TotalSettlement = DtransList.Sum(x => x.TotalAmount).Value;
                //if (SaleList.Count > 0)
                //{
                //List<string> _type = new List<string> { "Sale", "Credit", "GiftCard", "Settlement" };
                //var _tranIdList = SaleList.Where(x => _type.Contains(x.Type)).Select(x => x.TranId).ToList();
                //var settlementList = entity.Transactions.Where(x => x.DateTime.Value.Month == month && x.DateTime.Value.Year == year && x.Type == "Settlement").Select(x => x.TotalAmount).ToList();
                ...
                var cashAmount = (TotalCash + RecieveCredit + TotalSettlement) - (bankSettlementAmt + bankRecieveCreditAmt);
                var Bank = bankRecieveCreditAmt + bankRecieveCashAmt + bankSettlementAmt;
                SaleRevenue =Convert.ToInt32(cashAmount + Bank);
        ...
        private void salebreakdowncategory()
        {
            ...
            foreach (SaleBreakDownController s in ResultList)
            {
                ...
                s.BreakDown = (s.Sales == 0) ? 0 :((s.Sales / total) * 100);
        ...
        private void toptenproduct()
        {
            ...
            resultList = entity.Top100SaleItemList(fromDate, toDate, true, totalRow, SettingController.DefaultShop.ShortCode);
            foreach (Top100SaleItemList_Result r in resultList)
            {
                ...
                if (itemtopList.Where(x => x.ProductId == r.ProductCode).FirstOrDefault() != null)
                {
                    ...
                    p.totalAmount += Convert.ToInt32(r.Amount);
