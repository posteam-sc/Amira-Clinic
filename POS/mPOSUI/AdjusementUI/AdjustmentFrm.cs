...
namespace POS
{
    public partial class AdjustmentFrm : Form
    {
        ...
        private void btnAdjustment_Click(object sender, EventArgs e)
        {
            ...
            var productQty = entity.Products.Where(x => x.Id == productID).FirstOrDefault();
            if (cboSign.SelectedIndex == 0)
            {
                if (productQty != null)
                {

                    if (productQty.Qty < Convert.ToInt32(txtAdjustmentQty.Text.Trim()))
                    {
                        tp.SetToolTip(txtAdjustmentQty, "Error");
                        tp.Show("Adjustment Qty Not be grater Stock Qty!", txtAdjustmentQty);
                        HaveError = true;
                    }
                }
            }
        ...
        private void Bind_Product()
        {
            ...
            var _productList = (from p in entity.Products where p.IsService==false && p.IsPackage==false select p).ToList();
