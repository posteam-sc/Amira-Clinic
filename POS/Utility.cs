...
namespace POS
{
    class Utility
    {
        ...
        public static void BindConversionProduct(ComboBox cboProduct, string unitType)
        {
            ...
            productObj.Name = "ALL";
            productList.Add(productObj);
            productList.AddRange(entity.Products.Where(x => x.UnitType == unitType && (x.IsService == false || x.IsService == null) && x.IsPackage == false).ToList());
            cboProduct.DataSource = productList;
            cboProduct.DisplayMember = "Name";
            cboProduct.ValueMember = "Id";
            cboProduct.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
        ...
        public static int Opening_Run_Balance(int _year, int _month, long productId)
        {
            ...
            int _openingBalance = 0;
            int _preMonth;
            var stockTranList = new POS.APP_Data.StockTransaction();
            stockTranList = (from o in entity.StockTransactions where o.ProductId == productId && o.TranDate !="" select o).OrderByDescending(x => x.Year).ThenByDescending(x => x.Month).FirstOrDefault();
            //string tranDate = Month_Name(_preMonth, _year);

            //var stockTranList = (from o in entity.StockTransactions where o.ProductId == productId && o.TranDate==tranDate select o).OrderBy(x => x.Year).OrderByDescending(x => x.Month).FirstOrDefault();
